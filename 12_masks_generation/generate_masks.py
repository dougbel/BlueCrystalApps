...
class MasterMasksGeneration(object):
    """
    ...
    def run(self):
        ...
        dir_output_masks = f"{dir_output}/annotation_{self.stage}_images"
...
class SlaveMasksGeneration(Slave):
    ...
    def __init__(self, scannet_analysis_dir, stage, interactions_set, output_dir):
        ...
        self.dir_output_masks = f"{dir_output}/annotation_{stage}_images"
    ...
    def do_work(self, data):
        ...
        try:
            ...
            orig = os.path.join(self.full_dir_data, "frame_img_samplings", scene, "w224h224s10",
                                f"image_frame_{frame}_input.png")
            dest = os.path.join(self.dir_output_images, scene + str(frame).zfill(4) + '.png')
            os.link(orig, dest)

            masks = np.zeros((224, 224, len(self.interactions)))
            ...
            for pos in range(len(self.interactions)):
                ...
                ann_img = np.zeros((224, 224)).astype('uint8')
                ann_img[np.where(scores > self.threshold)] = 1
                masks[:, :, pos] = ann_img

            file = os.path.join(self.dir_output_masks, scene + str(frame).zfill(4))
            np.save(file, masks)
...
if __name__ == "__main__":
    ...
    if rank == 0:  # Master
        ...
        app = MasterMasksGeneration(slaves=range(1, size),
