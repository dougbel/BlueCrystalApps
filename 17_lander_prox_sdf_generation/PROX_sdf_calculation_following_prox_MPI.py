...
class MasterRoutines(object):
    ...
    def run(self):
        ...
        #NOTE: in past version I extend the bounding box in a proportion of 1.5, now I am doing that in 2.4 meters
        bb_padded = trimesh.primitives.Box(transform=transform, extents=trimesh_scan.extents + 2.4, mutable=False)
        ...
        # to visualizae uncomment this
        # import vedo
        # voxel_points = points.reshape(self.grid_dim * self.grid_dim * self.grid_dim, 3)
        # v = vedo.Plotter()
        # v.add(vedo.trimesh2vtk(trimesh_scan))
        ...
        json_output_file = os.path.join(self.output_dir, self.scene_name + ".json")

        if os.path.exists(json_output_file):
            logging.info(f'SDF values ALREADY calculated  for {self.scene_name}')
            return
        ...
        while not self.work_queue.done():
            ...
            for slave_return_data in self.work_queue.get_completed_work():
                ...
                if done:
                    ...
                    self.work_queue.add_work(data=[data[0], data[1], points[data[0], data[1], :]])
                    logging.info(f'REQUEING sdf calculation in x: {data[0]} , y: {data[1]}')
...
if __name__ == "__main__":
    ...
    # mpiexec -n 2 python 17_prox_matterport_replica_sdf_generation/PROX_sdf_calculation_following_prox_MPI.py --scans_dir /media/dougbel/Tezcatlipoca/Lander/dataset/prox/scene_artefact_removed --scene BasementSittingBooth --grid_dim 10 --output_dir /media/dougbel/Tezcatlipoca/Lander/dataset/prox/scene_artefact_removed/sdf_10
    # import pydevd_pycharm
    # port_mapping = [42009, 35821]
